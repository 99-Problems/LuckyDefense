...
public class PoolManager : MonoBehaviour
{
    ...
    Dictionary<string, Pool<EffectGameObject>> effectObjectPool = new Dictionary<string, Pool<EffectGameObject>>();
    ...
    public void PushCollider(ProjectileLogic mit)
    {
        if(mit.skillID > 0)
        {
            var _effect = mit.gameObject.GetComponentInChildren<EffectGameObject>();
            if(_effect != null)
            {
                PushEffectGameObject(_effect);
            }
        }
    ...
    public async UniTask<EffectGameObject> CreateEffectGameObjectPool(int skillID)
    {
        var key = String.Concat("common/effect/", skillID);
        var loadedObject = await Managers.Resource.LoadAsyncGameObject(
            $"common/effect/",
            $"{skillID}.prefab");
        ...
        if (effectObjectPool.TryGetValue(key, out var obj))
            return obj.Pop(null);

        var clone = GameObject.Instantiate(loadedObject).GetOrAddComponent<EffectGameObject>();
        clone.gameObject.SetActive(false);
        clone.skillID = skillID;

        List<GameObject> removeClone = new List<GameObject>();
        {
            var origin = loadedObject.GetComponent<EffectGameObject>();
        }
        await UniTask.DelayFrame(5);
        removeClone.Destroy(true);

        if (effectObjectPool.TryGetValue(key, out var pool))
        {
            return clone;
        }

        var newPool = new Pool<EffectGameObject>();
        newPool.Init(clone, $"skill_{skillID}", 1);
        newPool.Root.transform.SetParent(_root);

        effectObjectPool[key] = newPool;
        return newPool.Pop(null);
    }

    public EffectGameObject PopEffectGameObject(int skillID)
    {
        var key = String.Concat("common/effect/", skillID);
        if (effectObjectPool.TryGetValue(key, out var pool))
        {
            var effectObject = pool.Pop(null);
    ...
    public void PushEffectGameObject(EffectGameObject obj)
    {
        if (obj == null) return;

        var key = String.Concat("common/effect/", obj.skillID);
        if (effectObjectPool.TryGetValue(key, out var pool))
        {
        ...
        Debug.Log($"Destroy effect{obj.name}", Color.red);
    }
}